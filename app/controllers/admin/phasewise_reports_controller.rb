class Admin::PhasewiseReportsController < ApplicationController
	...
	def index
		...
		temp_load_data = RakeLoad.where(release_date: from_date..to_date,rakeform_otherform: "R")
		temp_unload_data = RakeUnload.where(release_date: from_date..to_date,form_status: ["RAKE","GETS","AECS"])

		@phasewise_commodity_list = (temp_load_data.map{|e| [e.major_commodity.major_commodity,e.major_commodity_id]}.uniq+temp_unload_data.map{|e| [e.major_commodity.major_commodity,e.major_commodity_id]}.uniq).uniq
		...
		if params[:is_data_filter].present?
			...
			rake_load_data = temp_load_data.select{|x| x if selected_commodities.include?(x.major_commodity_id.to_s)}
			rake_unload_data = temp_unload_data.select{|x| x if selected_commodities.include?(x.major_commodity_id.to_s)}

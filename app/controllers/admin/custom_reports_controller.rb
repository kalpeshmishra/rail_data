  ...
  def index
    from_date = params[:from_date].to_date if params[:from_date].present?
		...
		if params[:is_station_filter].present? and params[:selected_stations].present?
     	# stations = params[:selected_stations].split(",").map{|code| Station.find_by(code: code).id rescue nil}.compact.flatten.uniq
		...
		if params[:is_data_filter].present?
			...
			custom_report_data_hash = RakeLoad.get_custom_report_loading(custom_rake_load_data)
			# binding.pry
